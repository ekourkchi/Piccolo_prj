...
def convert_to_raw(value: float, lsb_value: float = 0.001) -> int: #0.001 as given in max32664 sensor datasheet which is used in WeBe
    ...
    if raw_value < 0:
        raw_value += (1 << 16)  # Convert to unsigned two's complement

    msb = (raw_value>>8)&0xFF 
    lsb = (raw_value)&0xFF
    return msb,lsb  # raw bytes return
...
def process_csv(input_file: str, output_dir: str):
    ...
    try:
        ...
        if not required_columns.issubset(df.columns):
            ...
            return
       # Convert accel values to raw bytes
        df[['accelx_msb', 'accelx_lsb']] = df['accelx'].apply(lambda v: pd.Series(convert_to_raw(v)))
        df[['accely_msb', 'accely_lsb']] = df['accely'].apply(lambda v: pd.Series(convert_to_raw(v)))
        df[['accelz_msb', 'accelz_lsb']] = df['accelz'].apply(lambda v: pd.Series(convert_to_raw(v)))

        # Drop original columns if needed
        df.drop(columns=['accelx', 'accely', 'accelz'], inplace=True)
